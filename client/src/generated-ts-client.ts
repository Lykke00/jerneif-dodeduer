...
export class GameClient {
    ...
    getGameDetails(gameId: string | undefined): Promise<ResultOfGameExtendedDto> {
        let url_ = this.baseUrl + "/api/Game/details?";
        if (gameId === null)
            throw new globalThis.Error("The parameter 'gameId' cannot be null.");
        else if (gameId !== undefined)
            url_ += "gameId=" + encodeURIComponent("" + gameId) + "&";
        ...
        return this.http.fetch(url_, options_).then((_response: Response) => {
            return this.processGetGameDetails(_response);
        });
    }

    protected processGetGameDetails(response: Response): Promise<ResultOfGameExtendedDto> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
            let result200: any = null;
            result200 = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver) as ResultOfGameExtendedDto;
            return result200;
            });
        } else if (status !== 200 && status !== 204) {
            return response.text().then((_responseText) => {
        ...
        return Promise.resolve<ResultOfGameExtendedDto>(null as any);
    }
}
...
export interface ResultOfGameExtendedDto {
    success: boolean;
