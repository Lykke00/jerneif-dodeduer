...
public static class GenerateApiClientsExtensions
{
    public static async Task GenerateApiClientsFromOpenApi(this WebApplication app, string path)
    {
        ...
        var documentationDirectory = Path.Combine(Directory.GetCurrentDirectory(), "Documentation");
        var openApiPath = Path.Combine(documentationDirectory, "openapi-documentation.json");
